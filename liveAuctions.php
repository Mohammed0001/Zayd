    ...
    <section class="products" id="products">
        ...
        <div class="content">
            ...
            if (isset($_GET["category"])) {
                $sql = "SELECT p.id, p.name, p.currentBid, p.bidExpiry, (SELECT COUNT(DISTINCT userID) FROM bid WHERE productID = p.id) AS bidders_count , (SELECT `file` FROM pimage WHERE pID = p.id AND isPrimary = '1') AS mainImage FROM product p WHERE `status` = 'active' AND `categoryName` = '" . $_GET["category"] . "';";

            } else {
                $sql = "SELECT p.id , p.name, p.currentBid, p.bidExpiry, (SELECT COUNT(DISTINCT userID) FROM bid WHERE productID = p.id) AS bidders_count , (SELECT `file` FROM pimage  WHERE pID = p.id AND isPrimary = '1') AS mainImage FROM product p WHERE `status` = 'active'";
